...
Everything above measures what mediation prevents; R60 measures what it
costs, so the write-up (R61) can state the price of the reference
monitor next to its coverage.

- **Two instruments, one timer.** `overhead_benchmark` reuses the R12
  `bench.benchmark` harness (perf_counter_ns, un-timed warmup,
  nearest-rank percentiles). The *mediation ladder* times one trivial
  tool call through `Gateway.execute`, adding one feature per rung
  (bare allow policy → taint label + spec + taint rule → history +
  chain rule → audit dispatch, plus the R57 value-ledger round-trip on
  its own), so each feature's cost is a readable delta. The *scaling
  sweeps* time pure `Gateway.decide` while one workload axis grows:
  worst-case rule count (only the last of *N* rules matches), history
  length (a `no_prior` matcher that must walk all *H* entries), and
  taint-set size (*S* sources joined and inspected).
- **Seeded synthetic workloads.** Rule names, history sources, and
  taint sources derive from `random.Random(seed)`; the same seed
  measures a byte-identical workload, and the pinned tables in
  `docs/benchmarks/overhead.md` are regenerated by one command.
- **The finding.** Full mediation costs ~12 µs per call on commodity
  cloud hardware, dominated by the fixed decide path — each bookkeeping
  feature rides at ≲1–2 µs; scaling is linear in scanned rules
  (~0.46 µs/rule) and history entries (~0.48 µs/entry) and near-flat in
  taint sources (~0.05 µs/source, C-level set algebra). At the sizes
  the R55/R56/R59 benchmarks actually use, overhead stays under ~25 µs
  — invisible against any tool that performs I/O.
- **CI stays stable by design.** Tests assert structure exactly (the
